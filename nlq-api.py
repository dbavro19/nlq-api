...
st.set_page_config(page_title="MKTX-API", page_icon=":tada", layout="wide")

#Headers
with st.container():
    st.header("CP+ Natural Languege Queries")
    st.subheader("Natural Languege Queries to CP+ API Calls")
...
You are an Finnancial Assistant that help translate user questions into API calls to get infomration about finnacial instruments 
Based on the provided question, categorize the request into one of the following categories of API endpoints:

<categories>
Candles - For getting historical price candles for a stock.
Quotes - For getting real-time price quotes for a stock.
Earnings - For getting historical earnings per share data or a future earnings calendar for a stock
N/A - If the question does not fit into any of the categories
Not Supported - For When the question is asking multiple questions that would require multiple API calls or multiple endpoints
<categories>
...
Return only one of the categories (Candles, Quotes, Earnings, N/A, or Not Supported) in <category> xml tags. No other text
...
You are an Finnancial Assistant that help translate user questions into API calls to get infomration about finnacial instruments 
Based on the provided question, extract the target company's Ticket symbol, Compnay ID, or Securities ID and use it to format a valid API url based on the fomrat provided
...
<api_format>
https://api.marketdata.app/v1/stocks/quotes/(Instrument ID)/
</api_format>



Return only one the formted API call in an <api> xml tag. No other text
...
You are an Finnancial Assistant that helps answer user questions about finnacial instruments using the payload of an API call
Based on the data provided, proivde a succinct answe to the user's question
Use the provided API response information for reference to the data's format and attributes
Think through your asnwer and be as accurate as possible
Output your thought process for everystep in <think_space> xml tags
Only use the provieded data to answer the question, dont make assumptions
...
Return your answer to the user's question in a conversational manner in an <answer> xml tag.
...
"""
    ...
    result=parse_xml(llmOutput, "answer")
